...
import { useGetImprovementLogsQuery } from "@/Store/api/getImprovementLogsApi/getImprovementLogsApi";
...
function ActivityTimeLine() {
  ...
  const { data, isLoading } = useGetImprovementLogsQuery({
  ...
  const renderLogContent = (log: WorkLog) => {
    ...
    const getValueColor = (value: string, logType: WorkLog["name"]) => {
      ...
      if (logType === "status_change") {
        ...
        if (value === "proposed") return "text-yellow-500";
      }
