...
import { useGetIncidentLogsQuery } from "@/Store/api/getIncidentLogsApi/getIncidentLogsApi";
...
function ActivityTimeLine() {
  ...
  const { data, isLoading } = useGetIncidentLogsQuery({
  ...
  const renderLogContent = (log: WorkLog) => {
    ...
    const getValueColor = (value: string, logType: WorkLog["name"]) => {
      ...
      if (logType === "status_change") {
        if (value === "investigating") return "text-orange-500";
        if (value === "resolved") return "text-green-500";
      }
